<%@ page import="com.wwcai.crm.settings.domain.DicValue" %>
<%@ page import="java.util.List" %>
<%@ page import="java.util.Map" %>
<%@ page import="java.util.Set" %>
<%@ page import="com.wwcai.crm.workbench.domain.Tran" %>
...
<script type="text/javascript">
			...
			$.ajax({
				...
				success : function (data) {
					...
					if(data.success) {
						...
						var html = "";
						html += '<div id="' + data.tr.id +
								'" class="remarkDiv" style="height: 60px;">';
						html += '<img title="zhangsan" src="image/user-thumbnail.png"style="width: 30px; height:30px;">';
						html +=
								'<div style="position: relative; top: -40px; left: 40px;" >';
						html += '<h5 id="e' + data.tr.id + '">' +
								data.tr.noteContent + '</h5>';
						html +=
								'<font color="gray">交易</font> <font color="gray">-</font> <b>${t.customerId}-${t.name}</b> <small style="color: gray;" id="e' + data.tr.id + '"> ' + data.tr.createTime + ' 由' + data.tr.createBy + '</small>';
						html +=
								'<div style="position: relative; left: 500px; top: -30px; height: 30px; width: 100px; display: none;">';
						html +=
								'<a class="myHref" href="javascript:void(0);"onclick="editRemark(\'' + data.tr.id + '\')"><span class="glyphicon glyphicon-edit" style="font-size: 20px; color: #E6E6E6;"></span></a>';
						html += '&nbsp;&nbsp;&nbsp;&nbsp;';
						html +=
								'<a class="myHref" href="javascript:void(0);" onclick="deleteRemark(\'' + data.tr.id + '\')"><span class="glyphicon glyphicon-remove" style="font-size: 20px; color: #E6E6E6;"></span></a>';
						html += '</div>';
						html += '</div>';
						html += '</div>';
	...
	function changeStage(stage, i) {
		...
		$.ajax({
			...
			success : function (data) {
				...
				/*
					data
						{"success":false/true,"t":{交易}}
				 */

				if(data.success) {
					// 成功后需要在详细信息页局部刷新 刷新阶段、可能性、修改人、修改时间
	...
	function changeIcon(stage, index1) {
		...
		// 如果当前阶段可能性不为0，前七个有可能是绿圈，绿色标记，黑圈，后两个为黑×
		} else {
		...
		$.ajax({
			...
			success : function (data) {
				...
				$.each(data, function (i, n) {
					...
					html +=
							'<font color="gray">交易</font> <font color="gray">-</font> <b>${t.customerId}-${t.name}</b> <small style="color: gray;" id="s' + n.id + '"> ' + (n.editFlag == 0 ? n.createTime : n.editTime) + ' 由' + (n.editFlag == 0 ? n.createBy : n.editBy) + '</small>';
					html +=
							'<div style="position: relative; left: 500px; top: -30px; height: 30px; width: 100px; display: none;">';
					html +=
...
<div class="modal fade" id="editRemarkModal" role="dialog">
	...
	<div style="position: relative; left: 40px; top: -50px;">
		...
		<%
			...
			if("0".equals(currentPossibility)) {
				...
				for(int i = 0; i < dvList.size(); i++) {
					...
					// 如果遍历出来的阶段可能性为0，说明 是后两个 一个红× 一个黑×
		...
		%>
							<span id="<%=i%>"
								  ...
								  class="glyphicon glyphicon-remove mystage"
								  data-toggle="popover"
								  data-placement="bottom" data-content="<%=dv.getText()%>"
	...
	<div style="position: relative; top: 0px;">
		...
		<div style="position: relative; left: 40px; height: 30px; top: 20px;">
			...
			<div style="width: 300px;position: relative; left: 200px; top:
			-20px;"><b>${t.customerId}</b></div>
			<div style="width: 300px;position: relative; left: 450px; top: -40px; color: gray;">阶段</div>
			<div style="width: 300px;position: relative; left: 650px; top:
			-60px;"><b id="stage">${t.stage}</b></div>
			<div style="height: 1px; width: 400px; background: #D5D5D5; position: relative; top: -60px;"></div>
			<div style="height: 1px; width: 400px; background: #D5D5D5; position: relative; top: -60px; left: 450px;"></div>
		</div>
		...
		<div style="position: relative; left: 40px; height: 30px; top: 70px;">
			...
			<div style="width: 500px;position: relative; left: 200px; top:
			-20px;"><b id="editBy">${t.editBy}&nbsp;&nbsp;</b><small
			style="font-size: 10px; color: gray;"
			id="editTime">${t.editTime}</small></div>
			<div style="height: 1px; width: 550px; background: #D5D5D5; position: relative; top: -20px;"></div>
		</div>
		...
		<div style="position: relative; left: 40px; height: 30px; top: 100px;">
			...
			<div style="width: 500px;position: relative; left: 200px; top:
			-20px;"><b>${t.nextContactTime}&nbsp;</b></div>
